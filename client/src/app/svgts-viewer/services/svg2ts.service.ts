...
export class Svg2TsService {
  ...
  public determineColorMode(icon: SVG2TSFile): "multiple" | "single" {
    ...
    return new Set(matches ? matches.map(_ => _.replace(/ /g, "")) : null)
      .size > 1
      ? "multiple"
      : "single";
  }
  ...
  public getTintInnerHtml(icon: SVG2TSFile, baseColor: string) {
    ...
    const colors = new Set(
      matches ? matches.map(_ => _.replace(/ /g, "")) : null
    ).size;
    let svgOutput = "";
    if (colors === 0) {
  ...
  private _getDynamicIcon(
    iconFile: SVG2TSFile,
    context = JSON.parse(JSON.stringify(iconFile.contextDefaults))
  ) {
